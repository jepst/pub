...
Haskell doesn't have an atom data type, but an idiomatic way of representing the different messages is to use type constructors. Imagine that a process should be able to perform mathematical operations remotely, and should be able to respond to two values: \textt{Add pid a b}, and \textt{Divide pid num den}. In response, the client process will be sent value \textt{Answer Double} or a \textt{DivByZero} error message if there was an attempt to divide by zero. One could create a message type encapsulating all these variants, suitable for use with \textt{expect}:
...
The problem with using a sum type, as above, is that \textt{expect} can only filter messages by type. Thus, a process receiving \textt{MathOps} messages would need to be able to respond to all variants, even those that don't make sense: presumably, only client processes should receive \textt{Answer} and \textt{DivByZero} while only the calculating process should receive \textt{Add} and \textt{Divide}. Putting all messages into a single data type breaks modularity by exposing details of the process to the client; when we add a new mathematical operation, every client will need to update its code, even if it doesn't use that operation. 

In order to avoid this kind of false dependency, it's better to break the single \textt{MathOp} type into several types which can be handled separately. We can imagine three types of messages:
...
In addition to the above types, the answer can be sent to the client simply as a message of type \textt{Double} --- no wrapper required. But now we have a different quandary: although \textt{expect} can receive any type of message, it can only receive one type of message at a time, and will block until a message of that type is put into the message queue. So there's no way to handle the above message types if we don't know the order that the client is going to send \textt{Add}s and \textt{Divide}s. In other words, we need an alternative to \textt{expect} that provides the notion of {\em choice}.
...
How, then, can we represent the multi-pronged choices of Erlang's \textt{receive} syntax? 
First, let's consider how to specify a pair consisting of a message type and its corresponding action. We introduce \textt{match}, which accepts a message handling function as a parameter.
When a message of the appropriate type is received, the handler function should be applied to it. 

\begin{code}
match :: (Serializable a) => (a -> ProcessM q) -> MatchM q ()
\end{code}

When \textt{match} tests an incoming message, it is comparing the type of the incoming message against the type \textt{a}, the parameter of the message handler. 
Any message of that type will be considered ``matched'': 
it will be removed from the message queue, and given as an argument to the message handling function.
...
One can see an analogy between the \textt{expect} function and its channel-based counterpart, \textt{receiveChan}: both receive a message of a particular type. Where, then, is the channel counterpart of \textt{receiveWait}? This is an important issue if we need to respond to messages on several channels, and we don't know on which channel the first will arrive. We expose this functionality to the user through the \textt{mergePorts} family of functions.
...
\item The implementation for the \textt{mergePorts} family of function was tricky to implement. It would have been simple, and wrong, to just fork a separate thread to call \textt{receiveChan} on each of several \textt{ReceivePort}s, but this causes an unfortunate race condition: imagine what would happen if messages are received simultaneously on two threads and extracted concurrently. Only one can be returned, and because there's no way to put messages back in the channel in the original order, we would need to either re-order the message or discard it.

Haskell's Software Transactional Memory (STM) helps us with this. STM provides a mechanism to compose receive transactions on individual message queues. 
...
systems. Specific to Java is the Remote Method Invocation (RMI)
...
undesirable becuase its cost model is hidden.  

Our operations on channels are somewhat reminiscent of Concurrent ML's events \cite{reppy:book},
but CML does not support distribution because that is incompatible with its
model of synchronisation.  paraML, an experimental variant of CML, did have
a distributed implementation; its serialisation mechanism was entirely built-in \cite{bailey:paraml}.
...
rebound rather than serialised.  HashCaml is (alpha-release)
implemented variant of OCaml that supports type-safe distributed
...
complete control of serialization, via tha usual type-class mechanism.
Exposing conversion is a significant cost, but one that also exposes the
cost model, and can in any case be ameliorated with syntactic sugar.
...
Jeff Epstein would like to give great thanks go to Alan Mycroft for his support and helpful feedback. 
