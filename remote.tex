...
\begin{itemize}
\item An interface for distributed programming in Haskell (Section \ref{s:processes}). Following the Erlang model, our framework provides a system for exchanging messages between concurrent processes, regardless of whether those threads are running on one computer or on many. We also provide functions for starting new threads remotely, and for fault tolerance, which closely follow Erlang. 
...
\begin{code}
  instance Serializable (types of the free variables of an a->b) => Serializable (a->b) where ...
\end{code}
...
We can hardly restrict \textt{spawn} to functions that don't have free variables, and even if we wanted to do so, there is no way to express that restriction.
...
\textt{newWorker} (at the start of this Section) we wanted to transmit closures
...
As a second example, consider \textt{newWorker} from the beginning of this section.
