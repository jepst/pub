...
When we say that the data to be transmitted must be serializable, we mean that each item must implement the \textt{Serializable} type class.  This ensures two properties: that it is \textt{Binary} and that it is \textt{Typeable} (see Figure~\ref{fig:api}).
...
The other problem with serializing functions is that it isn't enough to serialize just the function: we also need its environment, precisely, its free names. Some of the free names used by the \textt{ping} function, such as \textt{receiveWait}, are top-level. Assuming that the same code is running on all hosts (a nontrivial assumption), it's not necessary to transmit the value (that is, the actual code) of a top-level name, as we know that it already exists at the destination node. But consider how to serialize a function that has free names that are not top-level, such as this one:
...
\label{FaultTolerance}
Fault tolerance in Cloud Haskell borrows many good ideas from Erlang. Erlang's \textt{link} and \textt{monitor} functions allow processes to request notification if another process terminates. We provide analogous functions that offer similar functionality.
...
\end{code}
\apb{Maybe ``Additive'' rather than ``Add'', so simplify the discussion of whet happens if a second constructor is present (``Subtract'')?}

Clearly, \textt{expect} is not up to the task of translating the above code. 
\apb{This isn't so clear to me.  Can't I define \texttt{type M = Add | Divide | DivByZero}?  Then there is one type, a sum type, and I can check to see which element of the sum I have received using pattern matching.  This is not so different from Erlang, is it?}
\je{Yep, you can do that. As we discovered while playing around with POTS, you can always just combine your types. Thus receiveWait/match are convenient for cases where you don't want to or can't do that, but I'm not sure how to illustrate that.}
\textt{expect} can accept only one type of message, and will block until a message of that type is put in the message queue. In the interim, any other messages will simply accumulate in the message queue. How can we represent the branching structure of Erlang's \textt{receive} in Haskell?

First, let's consider how to specify a pair consisting of a message type and its corresponding action. We introduce \textt{match}, which accepts as a parameter a message handler. That message handler should be invoked when a message of the appropriate type is received. 

\begin{code}
match :: (Serializable a) => (a -> ProcessM q) -> MatchM q ()
\end{code}

When \textt{match} tests an incoming message, it is comparing the type of the incoming message against the type \textt{a}, the parameter of the message handler. Any message of that type will be considered matched, will be removed from the message queue, and will be given as a parameter to the message handler.
...
Matching by message type is not quite the same as matching by message value. For example, if a particular \textt{match} accepts message of a certain type, then all variants of that type must be handled. In the above example, this is okay, because the \textt{Add} type has only the \textt{Add} constructor. 
If instead it were a sum type with a second constructor, then the \textt{match} call that deals only with the \textt{Add} constructor would cause a pattern match exception if a message with the other constructor were received.
...
Whether we use \textt{receiveWait} directly or \textt{expect}, we run the risk that the function will block until a certain type of message arrives. What if we want to check the incoming message queue, but not wait indefinitely? Erlang lets us do this with the \textt{receive...after} syntax:
...
This code will wait at most 50 seconds to receive a response to its query; after 50 seconds, it will stop waiting and display an error message. Our framework's corresponding function is \textt{receiveTimeout}, which is very similar to \textt{receiveWait}. Like \textt{receiveWait}, it takes a list of \textt{match}es, but it also takes a timeout value. If the timeout is exceeded, the function returns \textt{Nothing}.
...
\textt{receiveTimeout} can also be called with a timeout value of zero, which has the effect of checking for a matching message in the queue and returning immediately if no match is found. This behavior also works with Erlang's \textt{receive...after} syntax.
...
How do we start the exchange? Clearly we need to create two channels and call \textt{ping2} and \textt{pong2} (not shown, but substantially similar to \textt{ping2}) as new processes. 
...
This \pair{code}{environment} pair is called a closure.


\apb{This doesn't fit here}

\subsection{Combining ports}
One can see an analogy between the \textt{expect} function and its channel-based counterpart, \textt{receiveChan}: both receive a message of one type. Where, then, is the channel counterpart of \textt{receiveWait}? This is an important issue if we need to respond to messages on several channels, and we don't know on which channel the first will arrive.

It would be simple, and wrong, to just fork a separate thread to call \textt{receiveChan} on each of several \textt{ReceivePort}s, but this causes an unfortunate race condition that may result in messages being extracted from a channel and then discarded without being processed. There's no way to put a message back in a channel once it's been read out, so we need some way to ensure that exactly one message will be extracted at a time from a group of $n$ \textt{ReceivePort}s.

Haskell's Software Transactional Memory (STM) helps us with this. STM provides a mechanism to compose receive transactions on individual message queues. We expose this functionality to the user through the \textt{mergePorts} family of functions.
...
Given a list of \textt{ReceivePort}s of the same message type, these functions will return a new \textt{ReceivePort} that, when read with \textt{receiveChan}, will provide a message taken from any of the input \textt{ReceivePort}s. You can visualize that these functions take several ``feeder'' ports and squeeze them together into a ``merged'' port. Even after producing the merged \textt{ReceivePort}, the original ports may continue to be used independently. Messages read from the merged port are extracted from the queue of the feeder port, and so it's impossible to receive the same message twice.

\textt{mergePortsBiased} and \textt{mergePortsRR} differ in the order that the input ports are queried, which is significant in the case that more than one port already has a message waiting. Each subsequent read from the port created by \textt{mergePortsBiased} will query the feeder ports strictly in the order that they were provided to \textt{mergePortsBiased}. Thus, the port is ``biased'' towards the first feeder port. In that case, if the first feeder port always has message waiting, it could starve the other ports.

To alleviate the problem of bias in merged ports, use \textt{mergePortsRR} instead. The port created by \textt{mergePortsRR}, unlike that created by \textt{mergePortsBiased}, will rotate the order in which the feeder ports are queried with each subsequent read. The effect is a fairer ``round-robin'' multiplexing, which will guarantee that, given enough reads on the merged port, every feeder port will eventually have a chance to contribute a message from its queue.

\je{Also mention combinePorts\{Biased,RR\} (which take ports of different types)? It seems like a digression.}
...
\end{code}
\textt{sf} is a function that creates an anonymous function and sends it on port \textt{p}.   The function that it sends, \lstinline!(\y -> x + y + 1)!,
...
\apb{using a library that knows only the type of the function itself}.
In concrete terms, there is no way to write this instance
declaration
\begin{code}
  instance ???? => Serializable (a->b) where ????
\end{code}
\apb{This doesn't help\,---\,too many question marks!}
...
We can't avoid this problem, because the essence of distributed computation is providing a way to start a remote computation; in a higher-order language like Haskell, that must inevitably
...
This approach is used by Glasgow Distributed Haskell \cite{gdh2001} and JoCaml~\cite{joCaml} \spj{check!!!}.
...
A similar approach is used by Java RMI~\cite{javarmi}; although RMI requires the programmer to determine which objects are serializable (by declaring that they implement the marker interface \textt{Serializable}), the programer is not required to actually write the methods that serialize the fields of the object; that is taken care of by the language implementation itself.
...
\apb{How is this different from\\
\textt{data Closure a = Closure (Static (e -> a))   e}
?  If I wrote it that way, the problem would still be with us, would it not?   Have existentials really got anything to do with this?}
Two different closures that have the same type may nonetheless capture environments
that have differing types.  For example, consider the list of closures \textt{cs}:
\begin{code}
  cs :: [Closure Int]
...
for how to serialize it!  
\apb{This explanation doesn't help, particularly because you the objection is a strawman that you dismiss in the next sentence.}
...
\item The low-level handling of message queues was implemented with Haskell's software transactional memory (STM) library. The ability to roll back unsuccessful transactions was important in implementing typed channels. In particular, our framework's \textt{mergePorts} family of functions allows the programmer to multiplex reads across multiple channels, possibly of different types. This functionality was surprisingly easy to implement with STM's \textt{orElse}.
...
Of the three options given in the above configuration files, only \textt{cfgRole} is strictly necessary. If \textt{cfgHostName} isn't specified, the framework will try to query its value from the operating system. Also, \textt{cfgKnownHosts} is optional, because \textt{getPeers} will try to automatically discover peers on the local network via UDP broadcast. Nevertheless, to ensure that the example applications works in as many networks as possible, it's safer to explicitly set these values.
