...
\lstset{language=Haskell, basicstyle=\footnotesize, numbers=none, numberstyle=\footnotesize, stepnumber=1, numbersep=5pt, showspaces=false, showstringspaces=false,showtabs=false, frame=none, tabsize=2, captionpos=b, breaklines=true, breakatwhitespace=false, xleftmargin=10pt,
...
One reason that this model of distributed computing has not been brought to Haskell before is that it requires a mechanism to run code on remote systems. To solve this, we present a novel method for serializing function closures. Our method does not require extending the compiler, but yet provides an intuitive syntax that hides the underlying serialization mechanism from the programmer.
...
Cloud Haskell provides two styles of message passing: \emph{untyped messages}, which closely follow the semantics of messages as laid out in Erlang; and \emph{typed channels}, which leverage Haskell's strong type system to provide static guarantees about the content of messages. In this section, we discuss untyped messages. We address channels in Section~\ref{s:Channels}.
...
If no patterns match, the unmatched message is left in the message queue, the whole receive statement is repeated for the next message, and so on until a match is found or the queue is exhausted.
...
We arbitrarily select the computer \textt{acs-01} to host the master node, and the computer \textt{acs-02} to host the worker node. The program, given below, should be compiled for both machines. Each machine also needs its own configuration file, named \textt{config}. Here is the configuration for \textt{acs-01}:
...
go "MASTER" = % \label{c_master}
  do aNode <- liftM (head . flip %\label{c_getpeer}
     ...
     cpid <- spawn aNode %\label{c_spawn}
                   ($(mkClosure 'counterLoop) 0) %\label{c_mkclo}
...
acquiring and loading the data; we hope to address this issue with improved file handling
in future versions.
...
Eden~\cite{eden2003} is an extension to Haskell that provides distributed process semantics. Unlike Cloud Haskell, its process abstraction and function serialization are built into the language, and therefore has no need of a process state monad. Although it lacks a mechanism for fault tolerance, it supports a rich library of higher-order functions as well as a debugging tools such a trace viewer.
