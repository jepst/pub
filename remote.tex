...
\textt{expect} can accept only one type of message, and will block until a message of that type is put into the message queue. In the interim, any other messages will simply accumulate in the message queue. How can we represent the branching structure of Erlang's \textt{receive} in Haskell?
...
It would be simple, and wrong, to just fork a separate thread to call \textt{receiveChan} on each of several \textt{ReceivePort}s, but this causes an unfortunate race condition that may result in messages being extracted from a channel and then discarded without being processed. 
\apb{Why?  I don't see the problem that you refer to.}
There's no way to put a message back in a channel once it's been removed, so we need some way to ensure that exactly one message will be extracted at a time from a group of $n$ 
\mbox{\textt{ReceivePort}s}.
